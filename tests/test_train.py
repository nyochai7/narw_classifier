...
from narw_classifier.training.train import _format_lr, default_run_name
...
class TestFormatLR:
    ...
    def test_compact_scientific(self, lr, expected):
        assert _format_lr(lr) == expected


class TestDefaultRunName:
    def _cfg(self, freeze_mode="linear_probe", lr=1e-3, batch_size=32, max_epochs=1):
